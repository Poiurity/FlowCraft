...
export class StaticValidator implements Verifier {
  ...
  private validateScreen(
    ...
    // appBar action checks (C3b; appBar is NOT in collectActions)
    if (screen.appBar?.actions) {
      for (let ai = 0; ai < screen.appBar.actions.length; ai++) {
        const a = screen.appBar.actions[ai].action;
        if (!a || a.type === 'none' || a.type === 'pop' || a.type === 'navigate') continue;
        const path = `screens[${si}].appBar.actions[${ai}].action`;
        this.checkAppBarAction(a, frame, path, emit);
      }
    }
  ...
  private checkTextBinding(
    ...
    while ((m = bindRe.exec(raw)) !== null) {
      ...
      // Step 3: dotted non-item — C1-dotted
      const head = expr.split('.')[0];
      if (expr.includes('.')) {
        const headDeclared = frame.varNames.has(head);
        if (!headDeclared) {
          // Escalate to error — head is undeclared
          ...
          emit(warn('C1-dotted',
            `Binding '{{${expr}}}' compiles to member access '_${head}.${expr.slice(head.length + 1)}' on state var '_${head}'. FlowCraft state vars have no members; this likely fails the Dart analyzer (Layer B).`,
            'static', { rawExpr: expr, head, emitted: `_${expr}`, headDeclared: true }, loc));
        }
        continue;
      }
  ...
  private checkAppBarAction(
    a: Action,
    frame: Frame,
    path: string,
    emit: (e: ValidationError) => void,
  ): void {
    if (!a || a.type === 'none' || a.type === 'pop' || a.type === 'navigate') return;
    const loc: SourceLocation = {
      screenId: frame.screen.id,
      screenIdx: frame.screenIdx,
      nodePath: path,
    ...
    const methodName = actionMethodName(a);
    // appBar actions are never collected by collectActions → always C3b
    const indexInjected = INDEX_REQUIRED.has(a.type);
    emit(err('C3b', `appBar action '${a.type}' emits a call to '${methodName}' but collectActions never visits appBar — method undeclared${indexInjected ? ' and references free `index`' : ''}`,
      'static',
      { actionType: a.type, problem: 'appBarUncollected', methodName,
        indexInjected, note: 'TODO: code-generator.ts:1004 (appBar action collection)' },
      loc));
  }
